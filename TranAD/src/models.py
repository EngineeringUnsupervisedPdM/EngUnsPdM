...
from TranAD.src.dlutils import *
...
class TranAD(nn.Module):
	def __init__(self, feats):
		...
		self.name = 'TranAD'
		self.lr = 0.001 #0.1 #for bearings
		# if feats<=13:
		# 	self.lr = 0.01
		self.batch = 128
		self.n_feats = feats
		...
		self.project_inp = nn.Linear(feats*2, self.dmodel)
		self.project_inp_target = nn.Linear(feats, self.dmodel)

		# original starts here
		self.pos_encoder = PositionalEncoding(self.dmodel, 0.1, self.n_window)
		encoder_layers = TransformerEncoderLayer(d_model=self.dmodel, nhead=8, dim_feedforward=16, dropout=0.1)
		self.transformer_encoder = TransformerEncoder(encoder_layers, 1)
		decoder_layers1 = TransformerDecoderLayer(d_model=self.dmodel, nhead=8, dim_feedforward=16, dropout=0.1)
		self.transformer_decoder1 = TransformerDecoder(decoder_layers1, 1)
		decoder_layers2 = TransformerDecoderLayer(d_model=self.dmodel, nhead=8, dim_feedforward=16, dropout=0.1)
		self.transformer_decoder2 = TransformerDecoder(decoder_layers2, 1)
		self.fcn = nn.Sequential(nn.Linear(self.dmodel, feats), nn.Sigmoid())

	def encode(self, src, c, tgt):
		src = torch.cat((src, c), dim=2)
		src = self.project_inp(src)
		...
		tgt = self.project_inp_target(tgt)
		return tgt, memory

	# Here we have model(window,elem)
	def forward(self, src, tgt):
		# Phase 1 - Without anomaly scores
		c = torch.zeros_like(src)
